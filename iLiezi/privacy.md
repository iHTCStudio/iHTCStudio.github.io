---
layout: doc
title: iLiezi — Privacy Policy
app_id: iLiezi
doc_title_en: Privacy Policy
doc_title_zh_cn: 隐私政策
doc_title_zh_tw: 隱私政策
description: Privacy policy for iLiezi — offline Liezi workshop, Data Not Collected, one classical text.
---
...
| Network | Library, reading, recite, Liezi Study, widgets, and daily notifications work **offline** after install |
| Analytics / ads / tracking | **None** — no third-party analytics, advertising, or tracking SDKs; no App Tracking Transparency use |
| Optional Apple services | **iCloud** (your private KVS), **Photo Library (Add Only)** for wind cards, **StoreKit** for optional tipping, **local notifications**, optional **Live Activities**, optional on-device **handwriting** (Vision) |
...
The app studies **one received text**. The Yang Zhu book is kept in full with a scholarly note: intellectual history, not a lifestyle slogan.
...
- **Widgets & App Group** — Widgets and the app share preferences via App Group `group.com.iHTCboy.iLiezi` on your device.
- **Daily notifications** — Scheduled locally. Tapping a notification opens a deep link on device.
- **Wind cards** — Images are rendered on device. Saving uses **Photo Library add-only** only when you tap save.
...
| Data | Where | Purpose |
|------|-------|---------|
| Liezi library | App / Widget bundle | Offline reading & study |
...
| Daily Liezi cache & notification prefs | App Group UserDefaults | Today’s ID and schedule |
...
iLiezi requests permissions only when a feature needs them:

| Permission | When | Why |
|------------|------|-----|
| Notifications | You enable Daily Liezi reminders | Local scheduling |
...
iLiezi is an educational literacy app for a general audience and is **not** in the Kids Category. We do not knowingly collect personal information from children. Families may use reading and study features offline on their own devices.
...
爱火腿肠工作室（iHTC Studio）（「我们」；原 iHTCTeam）开发了 **爱列子**（iLiezi / 愛列子，Bundle ID `com.iHTCboy.iLiezi`）。本隐私政策说明你在使用本应用时，数据如何被处理。本页可作为 **App Store 审核**与用户查阅的公开隐私政策。填写 App Store Connect 时：打开本站[隐私政策](privacy)页，从浏览器地址栏复制当前网址（站点域名日后可能变更，请勿依赖写死的完整域名）。
...
| 网络 | 语料、品读、诵读、列学学堂、小组件与每日通知安装后均可**离线** |
| 分析 / 广告 / 追踪 | **无** — 无第三方分析、广告或追踪 SDK；不使用 ATT |
| 可选 Apple 服务 | **iCloud**（你的私人 KVS）、保存御风卡时的**照片图库（仅添加）**、可选 **StoreKit** 打赏、**本地通知**、可选**实时活动**、可选本机**手写识别** |
...
本应用只做**这一部传世文本**。杨朱篇全文收录并加学术提示：这是思想史，不是生活口令。

## 应用如何工作

- **内置语料** — 正文 JSON、白话／英译、索引与学习辅助随 App／小组件打包。
- **本机语音** — 诵读使用系统 **AVSpeechSynthesizer**。我们不会把正文发送到 iHTC Studio。
- **小组件与 App Group** — 通过本机 App Group `group.com.iHTCboy.iLiezi` 共享偏好。
- **每日通知** — 本地调度；点按后经深链打开本机篇目。
- **御风卡** — 在设备上渲染。仅在你点保存时使用**照片图库（仅添加）**。
...
| 每日一列缓存与通知偏好 | App Group UserDefaults | 今日 ID 与提醒 |
...
爱列子是面向一般读者的教育类语文应用，**不属于** Kids Category。我们不会故意收集儿童个人信息。家庭可在自有设备上离线使用阅读与学习功能。
...
愛火腿腸工作室（iHTC Studio）（「我們」；原 iHTCTeam）開發了 **愛列子**（iLiezi / 爱列子，Bundle ID `com.iHTCboy.iLiezi`）。本隱私政策說明你在使用本應用時，資料如何被處理。本頁可作為 **App Store 審核**與使用者查閱的公開隱私政策。填寫 App Store Connect 時：打開本站[隱私政策](privacy)頁，從瀏覽器網址列複製目前網址（網站網域日後可能變更，請勿依賴寫死的完整網域）。
...
| 網路 | 語料、品讀、誦讀、列學學堂、小工具與每日通知安裝後均可**離線** |
| 分析 / 廣告 / 追蹤 | **無** — 無第三方分析、廣告或追蹤 SDK；不使用 ATT |
| 可選 Apple 服務 | **iCloud**（你的私人 KVS）、儲存御風卡時的**照片（僅加入）**、可選 **StoreKit** 打賞、**本機通知**、可選**即時動態**、可選本機**手寫辨識** |
...
本應用只做**這一部傳世文本**。楊朱篇全文收錄並加學術提示：這是思想史，不是生活口令。

## 應用如何工作

- **內建語料** — 正文 JSON、白話／英譯、索引與學習輔助隨 App／小工具打包。
- **本機語音** — 誦讀使用系統 **AVSpeechSynthesizer**。我們不會把正文傳送到 iHTC Studio。
- **小工具與 App Group** — 透過本機 App Group `group.com.iHTCboy.iLiezi` 共享偏好。
- **每日通知** — 本機排程；點按後經深鏈打開本機篇目。
- **御風卡** — 在裝置上渲染。僅在你點儲存時使用**照片（僅加入）**。
...
| 每日一列快取與通知偏好 | App Group UserDefaults | 今日 ID 與提醒 |
...
愛列子是面向一般讀者的教育類語文應用，**不屬於** Kids Category。我們不會故意蒐集兒童個人資訊。家庭可在自有裝置上離線使用閱讀與學習功能。
